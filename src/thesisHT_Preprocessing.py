...
alphabet = [chr(i) for i in range(ord('A'), ord('Z') + 1) if chr(i) not in {'P', 'Q', 'J', 'Z'}]
label_to_int = {letter: index for index, letter in enumerate(alphabet)}

# Display the predefined label mapping
print("Predefined Label Mapping:", label_to_int)

# Load data
print("Loading data...")
data = pd.read_csv(csv_file_path)

# Filter out unwanted labels ('P', 'Q', 'J', 'Z') and keep only relevant labels
print("Filtering unwanted labels...")
data = data[data.iloc[:, 0].isin(alphabet)]
...
# Encode labels using the predefined mapping
